...
do
	...
	./random_forest_ts.py train_file.csv test_file.csv | grep -v Warning > "$i"forest.txt
	./gradient_descent_ts.py train_file.csv test_file.csv | grep -v Warning > "$i"gradient.txt
	i=$((i + 1))
